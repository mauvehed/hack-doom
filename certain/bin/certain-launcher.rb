...
if !$nowebsocket then
  system("#{$wsbin} --loglevel=debug --port #{options[:websocketport]} ./certain.rb --iwad #{options[:iwad]} --assets #{options[:assets].join(' ')} --wadfiles #{options[:wadfiles].join(' ')} --marines #{options[:marines]} --level #{options[:level]}")
else
